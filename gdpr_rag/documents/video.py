...
class Video(Document):
    def __init__(self, path_to_manual_as_csv_file = "./inputs/documents/video.parquet"):
        ...
        if not self.check_columns():
            raise AttributeError(f"The input parquet file for the Video class does not have the correct column headings")
    ...
    class VideoReferenceChecker(ReferenceChecker):
        def __init__(self):
            exclusion_list = [""] 
            index_patterns = [
                r'^(\d+)', 
