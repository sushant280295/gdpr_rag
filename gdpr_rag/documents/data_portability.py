...
class DataPortability(Document):
    def __init__(self, path_to_manual_as_csv_file = "./inputs/documents/data_portability.parquet"):

        reference_checker =  self.DataPortabilityReferenceChecker()
        ...
        document_name = "Guidelines on the right to data portability"
    ...
    class DataPortabilityReferenceChecker(ReferenceChecker):
        def __init__(self):
            exclusion_list = ["Executive summary"] 
            index_patterns = [
                r'^\b(I|II|III|IV|V|VI|VII)\b',   
                r'^\.([a-z])', 
                r'^\.(\d+)', 
            ]    
            text_pattern = r'(I|II|III|IV|V|VI|VII)(\.([a-z]))?(\.(\d+))?'
