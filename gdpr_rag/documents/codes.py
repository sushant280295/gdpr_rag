...
class Codes(Document):
    def __init__(self, path_to_manual_as_csv_file = "./inputs/documents/codes.parquet"):

        main =  self.CodesReferenceChecker()
        appendix = self.AppendixReferenceChecker()
        reference_checker = MultiReferenceChecker([main, appendix])
        ...
        document_name = "Guidelines 1/2019 on Codes of Conduct and Monitoring Bodies under Regulation 2016/679"
        super().__init__(document_name, reference_checker=reference_checker)
        if not self.check_columns():
            raise AttributeError(f"The input parquet file for the Codes class does not have the correct column headings")
    ...
    class CodesReferenceChecker(ReferenceChecker):
        def __init__(self):
            exclusion_list = [""] 
            index_patterns = [
                r'^(\d+)', 
    ...
    class AppendixReferenceChecker(ReferenceChecker):
        def __init__(self):
            exclusion_list = [] #
            index_patterns = [
                r'^APPENDIX (\d+)'
            ]
            text_pattern = r'APPENDIX \d+'
