...
class Forgotten(Document):
    def __init__(self, path_to_manual_as_csv_file = "./inputs/documents/forgotten.parquet"):

        reference_checker =  self.ForgottenReferenceChecker()
        ...
        document_name = "Guidelines 5/2019 on the criteria of the Right to be Forgotten in the search engines cases under the GDPR (part 1)"
        super().__init__(document_name, reference_checker=reference_checker)
        if not self.check_columns():
            raise AttributeError(f"The input parquet file for the Forgotten class does not have the correct column headings")
